...
* Calculate: percentage of urban dvmt & percentage of urban lane miles in each state
* Indexing those percentages against national level
...
*Adjusting disbursements in 3 ways*: 
...
# Adjusted for 3 factors: cost of living, urban_dvmt_index, urban_lane_miles_index
SF_4_adjusted <- SF_4 %>% 
  left_join(coli) %>% 
...
Rural fatality category,  and urban fatality categories with each measuring fatalities on Interstates, OFA, and OPE highways.
Total
...
NOTE: VM2 file says unit = millions, should be thousand? --> multiple each columns * 1000 ?
...
method 1: adjusted for cost of living, divided by state total daily vehicle mile travelled
method 2: adjusted for urban_dvmt_index (percentage of urban vmt relatively to national), divided by state total daily vehicle mile travelled
method 3: adjusted for urban_lanemiles_index (percentage of urban lane mile relatively to national), divided by state total lane miles
...
```{r}
  ...
  select(state, capital_disbursement, maintenance_disbursement, admin_disbursement,
         capital_disbursement_pervmt_coli: admin_disbursement_per_lm)
...
disbursment_adjusted %>% arrange(desc(capital_disbursement)) %>% slice(1:10) %>% 
  ...
  scale_fill_manual(values = wesanderson::wes_palette("Royal1")) +
  theme_minimal()+
  labs(title = "Top 10 states that have highest capital_disbursement PER VMT") +
...
disbursment_adjusted %>% arrange(capital_disbursement) %>% slice(1:10) %>% 
  ...
  scale_fill_manual(values = wesanderson::wes_palette("Royal1")) +
  theme_minimal()+
  labs(title = "Top 10 states that have lowest capital_disbursement PER VMT") +
...
```{r}
## Method 0: Not adjusting for disbursement
method0 <- disbursment_adjusted %>% select(state, capital_disbursement, maintenance_disbursement, admin_disbursement) %>% left_join(metrics_9others) %>% 
...
# method 1: adjusted for cost of living - coli index
method1 <- disbursment_adjusted %>% select(state, capital_disbursement_pervmt_coli, maintenance_disbursement_pervmt_coli, admin_disbursement_pervmt_coli) %>% 
...
method2 <- disbursment_adjusted %>% select(state, capital_disbursement_per_vmt, maintenance_disbursement_per_vmt, admin_disbursement_per_vmt) %>% 
  left_join(metrics_9others) %>% 
  pivot_longer(cols = 2:13,
               names_to = "metrics",
               values_to = "value") %>% 
...
method3 <- disbursment_adjusted %>% select(state, capital_disbursement_per_lm, maintenance_disbursement_per_lm, admin_disbursement_per_lm) %>% 
  left_join(metrics_9others) %>% 
  pivot_longer(cols = 2:13,
               names_to = "metrics",
               values_to = "value") %>% 
...
ranking_data %>% slice(1:50) %>% 
  ...
  scale_fill_manual(values = wesanderson::wes_palette("Royal2")) -> p1
...
ranking_data %>% slice(51:100) %>% 
  ...
  #labs(title = "Comparison: 26th report ranking vs. new methods") +
  theme(legend.position = "bottom") +
  scale_fill_manual(values = wesanderson::wes_palette("Royal2")) -> p2
